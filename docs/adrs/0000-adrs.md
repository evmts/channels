...
**Problem:** Trad docs capture "what" lose "why". Requirements/specs/comments ≠ options/trade-offs/constraints. Need: lightweight (actually done), sufficient context (useful later), versioned (with code), integrated (workflow).
</context>
...
**Rule:** Multiple smart people could disagree? → ADR. One obvious way? → No ADR.
</when>

<how>
**Steps:** 1) Copy template 2) Fill metadata 3) Context (2-3¶) 4) Drivers (top 3-5) 5) Options (2-4: desc/pros/cons/code/effort) 6) Decision (what+why) 7) Consequences (pos/neg/mitigate) 8) Implementation (structure/tests/docs/migration) 9) Review (draft→feedback→discuss→update→Accepted) 10) Implement (reference `// ADR-XXX`, commits, update status)
...
**Transitions:** Proposed→Accepted/Deprecated, Accepted→Deprecated/Superseded
**Update:** ✅ Status change, changelog, "Superseded by" link | ❌ Edit decision (new ADR), delete deprecated, change dates
</status>
...
**PRs:** List ADRs: ✅ Implemented, 🔄 In progress

**Change:** New ADR "Supersedes ADR-XXX"→Update old "Superseded by ADR-YYY"→Changelog both
</workflow>
...
<best-practices>
**DO:** ✅ Before implement, concise, multiple opts, pros+cons, simple, examples, ref in code, update index
**DON'T:** ❌ Edit accepted (new instead), after-fact, too long, 1 opt, hide negatives, jargon, drift
...
├── ...
└── README.md (index: list/status/summary ✅🔄⚠️)
```
</structure>
